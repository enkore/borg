...
cdef class AES256_CTR_BLAKE2b(AES256_CTR_BASE):
    ...
    def __init__(self, mac_key, enc_key, iv=None, header_len=1, aad_offset=1):
        if len(mac_key) < 128:  # XXX dirty hack to get tests working - remove this when fixed XXX
            print("warning, mac key too short: %d" % len(mac_key))
            mac_key += b'\0' * (128 - len(mac_key))
...
def hmac_sha512(key, data):
    ...
    try:
        with nogil:
            rc = HMAC(EVP_sha512(), key_ptr, key_len, <const unsigned char*> data_buf.buf, data_buf.len, md_ptr, NULL)
        if rc != md_ptr:
            raise CryptoError('HMAC(EVP_sha512) failed')
    finally:
        PyBuffer_Release(&data_buf)
    return md
